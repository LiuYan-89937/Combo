...
class FactoryExtensionTest(unittest.TestCase):
    def test_factory_extension_manager_discovers_skill_tool(self) -> None:
        ...
        self.assertIn("skills", result.runtime_resources)
    ...
    def test_factory_tool_specs_merge_builtin_and_extensions(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            specs = get_factory_tool_specs(tool_ids=["skill"], extension_root=root)
...
def _write_skill_extension(root: Path, *, tool_id: str = "echo", prefix: str | None = "factory") -> None:
    ...
    (skill_root / "SKILL.md").write_text(
        "---\n"
        "name: factory\n"
        "description: Factory extension skill.\n"
        "---\n"
        "# Factory Skill\n",
        encoding="utf-8",
    )
    (skill_root / "references" / "usage.md").write_text("Use the factory skill.", encoding="utf-8")

Create Codex pet row `running-right` for `combo-companion`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Only the single small rounded black eighth-note companion character from the reference. It has two vertical white oval eyes and two short legs. Do not include the large lead note or the large curved accent stroke.. Preserve silhouette, face, palette, material, proportions, markings, and props.

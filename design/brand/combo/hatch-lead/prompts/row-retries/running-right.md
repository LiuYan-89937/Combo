Create Codex pet row `running-right` for `combo-lead`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Only the single large rounded black eighth-note character from the reference. It has two vertical white oval eyes, two short legs, and one separate curved black accent stroke behind its left side. Do not include the small companion.. Preserve silhouette, face, palette, material, proportions, markings, and props.

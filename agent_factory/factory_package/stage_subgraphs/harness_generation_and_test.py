...
from agent_factory.factory_package.sandbox_runtime import (
    SandboxRuntimeError,
    runtime_for_backend,
)
...
def run_harness_generation_and_test_subgraph(state: FactoryPackageState) -> dict[str, Any]:
...
def _initialize_harness_context(state: FactoryPackageState) -> dict[str, Any]:
...
def _harness_react_model(state: FactoryPackageState) -> dict[str, Any]:
...
def _finalize_harness_contracts(state: FactoryPackageState) -> dict[str, Any]:
...
def _validate_harness_contracts(state: FactoryPackageState) -> dict[str, Any]:
    harness = dict(state.get("harness_generation") or {})
...
def _prepare_sandbox_runtime(state: FactoryPackageState) -> dict[str, Any]:
    harness = dict(state.get("harness_generation") or {})
    decision = HarnessContractDecision.model_validate(harness.get("contract_decision") or {})
...
def _execute_harness_plan(state: FactoryPackageState) -> dict[str, Any]:
    harness = dict(state.get("harness_generation") or {})
    decision = HarnessContractDecision.model_validate(harness.get("contract_decision") or {})
...
def _validate_harness_results(state: FactoryPackageState) -> dict[str, Any]:
    harness = dict(state.get("harness_generation") or {})
...
def _publish_harness_report(state: FactoryPackageState) -> dict[str, Any]:
    harness = dict(state.get("harness_generation") or {})
...
def _route_after_harness_model(state: FactoryPackageState) -> str:
...
def _route_after_harness_decision(state: FactoryPackageState) -> str:
...
def _route_after_contract_validation(state: FactoryPackageState) -> str:
    if state.get("harness_report"):
        return "publish_harness_report"
    if state.get("status") == "failed" or state.get("graph_control", {}).get("action") == "end":
...
def _route_after_sandbox_prepare(state: FactoryPackageState) -> str:
    if state.get("harness_report"):
        return "publish_harness_report"
    return "execute_harness_plan"


def _route_after_harness_results(state: FactoryPackageState) -> str:
    if state.get("harness_report"):
        return "publish_harness_report"
    if state.get("harness_validation_observation"):
        return HARNESS_REACT_MODEL_NODE
...
def _contract_errors(decision: HarnessContractDecision, state: FactoryPackageState) -> list[HarnessReportError]:
...
def _terminal_report_patch(state: FactoryPackageState, status: str, *errors: HarnessReportError) -> dict[str, Any]:
...
def _sandbox_execution_observation(state: FactoryPackageState, report: HarnessValidationReport) -> dict[str, Any]:
    return {
        "attempt": int(state.get("harness_revision_attempt") or 1),
        "status": report.status,
...
def _emit_harness_tool_events(state: FactoryPackageState) -> dict[str, Any]:
    harness = dict(state.get("harness_generation") or {})
...
def _resource_values(state: FactoryPackageState) -> dict[str, Any]:
...
def _with_prepared_sandbox_contract(decision: HarnessContractDecision, state: FactoryPackageState) -> HarnessContractDecision:
...
def _package_dependencies_plan(state: FactoryPackageState) -> SandboxDependencyPlan | None:
    package_root = _package_root(state, str(state.get("factory_run_id") or "default"))
...
def _prepared_sandbox_contract(state: FactoryPackageState) -> SandboxContract | None:
    plan = dict(state.get("resource_condition_plan") or {})
...
def _package_root(state: FactoryPackageState, factory_run_id: str) -> Path:

...
def run_assembly_spec_generation_subgraph(state: FactoryPackageState) -> dict[str, Any]:
    return build_assembly_spec_generation_subgraph().invoke(state)


def _initialize_assembly_context(state: FactoryPackageState) -> dict[str, Any]:
...
def _assembly_react_model(state: FactoryPackageState) -> dict[str, Any]:
...
def _validate_assembly_draft(state: FactoryPackageState) -> dict[str, Any]:
...
def _publish_assembly_spec_draft(state: FactoryPackageState) -> dict[str, Any]:
...
def _fail_assembly_generation(state: FactoryPackageState) -> dict[str, Any]:
...
def _route_after_model(state: FactoryPackageState) -> str:
...
def _route_after_validation(state: FactoryPackageState) -> str:
...
def _stage_constraint_errors(spec: AgentAssemblySpec, state: FactoryPackageState) -> list[str]:
    errors: list[str] = []
...
def _with_system_runtime_contract(spec: AgentAssemblySpec, state: FactoryPackageState) -> AgentAssemblySpec:
    metadata = {
...
def _binding_contract_errors(spec: AgentAssemblySpec, state: FactoryPackageState) -> list[str]:
    errors: list[str] = []
...
def _required_binding_types_by_node(pattern_nodes: dict[str, dict[str, str]], state: FactoryPackageState) -> dict[str, set[str]]:
...
def _nodes_with_strategy_refs(state: FactoryPackageState) -> set[str]:
...
def _build_render_manifest(spec: AgentAssemblySpec, state: FactoryPackageState) -> RenderManifest:
    pattern_nodes = _pattern_nodes(spec.runtime.pattern_id)
...
def _validate_materialization_plan(plan: PackageMaterializationPlan, state: FactoryPackageState) -> PackageMaterializationValidationReport:
    errors: list[str] = []
...
def _tool_capabilities_by_id(state: FactoryPackageState) -> dict[str, dict[str, Any]]:
...
def _resource_keys_for_tool(tool_id: str, state: FactoryPackageState) -> list[str]:
...
def _updated_report(state: FactoryPackageState, attempt: AssemblyValidationAttempt) -> AssemblyValidationReport:
...
def _failed_report(state: FactoryPackageState) -> AssemblyValidationReport:
    existing = dict(state.get("assembly_validation_report") or {})
    attempts = [AssemblyValidationAttempt.model_validate(item) for item in existing.get("attempts", []) or []]
...
def _attempt_count(state: FactoryPackageState) -> int:
    report = dict(state.get("assembly_validation_report") or {})
...
def _graph_node_ids(state: FactoryPackageState) -> set[str]:
...
def _graph_behavior_nodes_by_id(state: FactoryPackageState) -> dict[str, dict[str, Any]]:
    graph_behavior = dict(state.get("graph_behavior_plan") or {})
    return {
...
def _node_strategies_by_id(state: FactoryPackageState) -> dict[str, dict[str, Any]]:

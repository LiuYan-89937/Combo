...
from agent_factory.factory_package.prompt_context import prompt_context_values, stage_operating_context
...
def run_requirement_capture_subgraph(state: FactoryPackageState) -> dict[str, Any]:
...
def _initialize_requirement(state: FactoryPackageState) -> dict[str, Any]:
...
def _judge_requirement_clarity(state: FactoryPackageState) -> dict[str, Any]:
    capture = dict(state.get("requirement_capture") or {})
    try:
...
def _extract_requirement_frame(state: FactoryPackageState) -> dict[str, Any]:
    capture = dict(state.get("requirement_capture") or {})
    try:
...
def _generate_clarifying_question(state: FactoryPackageState) -> dict[str, Any]:
    capture = dict(state.get("requirement_capture") or {})
...
def _wait_for_requirement_answer(state: FactoryPackageState) -> dict[str, Any]:
    capture = dict(state.get("requirement_capture") or {})
    answer = interrupt(
        {
...
def _merge_requirement_answer(state: FactoryPackageState) -> dict[str, Any]:
    capture = dict(state.get("requirement_capture") or {})
...
def _finalize_requirement(state: FactoryPackageState) -> dict[str, Any]:
    capture = dict(state.get("requirement_capture") or {})
...
def _route_after_clarity(state: FactoryPackageState) -> str:
...
def _route_after_structured_node(state: FactoryPackageState) -> str:
    if state.get("status") == "failed" or state.get("graph_control", {}).get("action") == "end":
        return END

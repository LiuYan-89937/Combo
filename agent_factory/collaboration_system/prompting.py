    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "delivery_standard 由你根据用户目标和 agent_search 返回的子 Agent 能力动态制定，只声明交付文件及语义验收标准 acceptance_criteria。宿主只检查文件路径安全、确由本轮产生且非空，不替你判断内容质量。",
            "acceptance_criteria 应描述可由你阅读产物后判断的业务结果，例如数据范围、来源说明、分析维度、异常披露和最终结论；不要写成关键词命中、固定标题或机械措辞。",
            ...
            "验收 submitted 任务时，必须使用 read_task_artifacts 并传 task_id 读取该任务的权威 artifact_refs，逐项对照 acceptance_criteria 作出语义判断；不要仅根据 result_summary、文件名或宿主基础校验直接通过。",
            "结果满足标准时用 update_task 将任务状态更新为 completed，并写明 review_notes；结果不满足标准时先更新为 revision_requested 并写清返工要求，再对同一 task_id 调用 retry_task。发现原 delivery_standard 本身不合理时，重跑必须同时提交修正后的 delivery_standard，不能要求 worker 用机械措辞迎合错误契约。",

...
def run_package_generation_subgraph(state: FactoryGraphState) -> dict[str, Any]:
...
def _initialize_package_context(state: FactoryGraphState) -> dict[str, Any]:
...
def _package_react_model(state: FactoryGraphState) -> dict[str, Any]:
...
def _finalize_package_build_decision(state: FactoryGraphState) -> dict[str, Any]:
...
def _validate_package_build_plan(state: FactoryGraphState) -> dict[str, Any]:
    plan = dict(state.get("package_generation") or {})
...
def _materialize_package_files(state: FactoryGraphState) -> dict[str, Any]:
    plan = dict(state.get("package_generation") or {})
    decision = PackageBuildDecision.model_validate(plan.get("build_decision") or {})
...
def _validate_package_structure(state: FactoryGraphState) -> dict[str, Any]:
    plan = dict(state.get("package_generation") or {})
...
def _publish_package_report(state: FactoryGraphState) -> dict[str, Any]:
    plan = dict(state.get("package_generation") or {})
...
def _route_after_package_model(state: FactoryGraphState) -> str:
...
def _route_after_package_decision(state: FactoryGraphState) -> str:
    if state.get("status") == "failed" or state.get("graph_control", {}).get("action") == "end":
        return END
    return "validate_package_build_plan"


def _route_after_package_validation(state: FactoryGraphState) -> str:
    if state.get("status") == "failed" or state.get("graph_control", {}).get("action") == "end":
        return END
...
def _route_after_build_plan_validation(state: FactoryGraphState) -> str:
    if state.get("status") == "failed" or state.get("graph_control", {}).get("action") == "end":
        return END
...
def _validate_decision(decision: PackageBuildDecision, state: FactoryGraphState) -> list[str]:
    errors: list[str] = []
...
def _validate_agent_package_manifest(package_root: Path, manifest_path: Path) -> list[str]:
    ...
    required_contracts = {"context", "dependencies", "model", "render", "resources", "sandbox", "scheduler", "session", "tools"}
...
def _resource_keys(state: FactoryGraphState) -> set[str]:
...
def _materialization_plan(state: FactoryGraphState, factory_run_id: str | None = None) -> PackageMaterializationPlan:
...
def _emit_package_tool_events(state: FactoryGraphState) -> dict[str, Any]:
    plan = dict(state.get("package_generation") or {})

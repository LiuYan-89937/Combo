"""
容器池管理器 - 实现跨 session 的容器复用，降低内存占用和启动延迟

核心思路：
1. 同一个 package 的多个 session 可以共享容器（通过 session_id 路由到不同的内部会话）
...
class ContainerPool:
    ...
    - 容器支持多 session 复用（通过 stdio_server 的 session_id 隔离）
    - 空闲容器保持温热，超时后清理
    """
    ...
    def __init__(
        ...
        *,
        ...
        # package_id -> deque of PooledContainer
        self._available_by_package: dict[str, Deque[PooledContainer]] = {}
    ...
    def acquire(
        ...
        *,
        ...
        with self._lock:
            ...
            # 1. 尝试复用相同 package 的空闲容器
            available = self._available_by_package.get(package_id)
            if available:
    ...
    def release(self, handle: AgentRuntimeContainerHandle) -> None:
        ...
        with self._lock:
            ...
            package_available = self._available_by_package.setdefault(pooled.package_id, deque())

            # 检查该 package 的池是否已满
            if len(package_available) >= self.max_containers_per_package:
                logger.debug(
                    "Package pool full, closing container: package=%s, pool_size=%d",
                    pooled.package_id,
                    len(package_available),
                )
                handle.close()
                return
            ...
            # 加入空闲池
            pooled.last_used = time.monotonic()
            package_available.append(pooled)
            logger.debug(
                "Released container to pool: package=%s, pool_size=%d",
                pooled.package_id,
                len(package_available),
            )
    ...
    def evict_package(self, package_id: str) -> int:
        ...
        with self._lock:
            ...
            # 清理空闲容器
            available = self._available_by_package.pop(package_id, deque())
    ...
    def close_all(self) -> None:
        ...
        with self._lock:
            ...
            # 关闭所有空闲容器
            for package_available in self._available_by_package.values():
                for pooled in package_available:
                    pooled.handle.close()
            self._available_by_package.clear()
    ...
    def stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            available_by_package = {
                pkg: len(containers)
                for pkg, containers in self._available_by_package.items()
            }
            ...
            return {
                "total_available": sum(len(c) for c in self._available_by_package.values()),
    ...
    def _total_container_count(self) -> int:
        """当前池中的总容器数（空闲 + 使用中）"""
        return len(self._in_use) + sum(len(c) for c in self._available_by_package.values())
    ...
    def _evict_least_recently_used(self) -> bool:
        ...
        oldest_package_id: str | None = None

        for package_id, available in self._available_by_package.items():
            for pooled in available:
                if oldest_pooled is None or pooled.last_used < oldest_pooled.last_used:
                    oldest_pooled = pooled
                    oldest_package_id = package_id

        if oldest_pooled is not None and oldest_package_id is not None:
            available = self._available_by_package[oldest_package_id]
    ...
    def _cleanup_idle_containers(self) -> None:
        ...
        with self._lock:
            ...
            for package_id, available in list(self._available_by_package.items()):
                for pooled in list(available):
                ...
                # 移除空队列
                if not available:
                    del self._available_by_package[package_id]

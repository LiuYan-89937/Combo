...
- If the only needed change is dependency metadata, use `create_agent_authoring(action="configure_dependencies")` with `python_requirements`, `system_packages`, `npm_requirements`, `system_binaries`, or `verification_commands`.

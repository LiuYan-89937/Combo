    ...
    def create(
        ...
        *,
        ...
        with self._database.transaction() as conn:
            _require_active_generation(conn, runtime_instance.generation, now=now)
...
def _require_active_generation(conn: sqlite3.Connection, generation: int, *, now: str) -> None:
    row = conn.execute(
        """
    ...
    if row is None:
        raise RuntimeError("runtime start generation is not active or its lease expired")

...
个人智能助手可以协调三个内置美股专业 Agent：盘面雷达负责主要指数、涨跌榜与活跃个股；上市公司研究员负责 Yahoo Finance 行情、SEC 公司事实和趋势证据；持仓风险管家负责集中度、波动率、回撤、标普 500 Beta、相关性和指定压力情景。主助手汇总各子任务，保存带数据来源、数据时间、异常说明和风险声明的报告，并可在用户授权后发送邮件。该示例用于展示复杂任务拆分、工具调用、多 Agent 协作和交付能力，不构成投资建议。

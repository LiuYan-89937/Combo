...
public final class ComputerUseService {
    ...
    public func click(
        ...
        debugClickDecision("context pid=\(snapshot.app.pid) window=\(String(describing: snapshot.targetWindowID)) bounds=\(String(describing: snapshot.windowBounds)) screenshotPixels=\(String(describing: screenshotPixelSize(snapshot: snapshot))) requested=\(clickMethod.rawValue)")
    ...
    public func typeText(app query: String, text: String, elementIndex: String? = nil) throws -> ToolCallResult {
        ...
        guard AXUIElementCopyAttributeValue(system, kAXFocusedUIElementAttribute as CFString, &focusedValue) == .success,
            ...
            throw ComputerUseError.message("[input.focus_mismatch] No live focused element is available")
        }
        let focused = focusedValue as! AXUIElement
        if let elementIndex {
            let target = try inputElement(snapshot: snapshot, index: elementIndex)
            guard CFEqual(target, focused) else {
                throw ComputerUseError.message("[input.focus_mismatch] The requested target is not the live focused element")
            }
        }
        try validateInputElement(focused, pid: snapshot.app.pid)
        let before = try inputText(focused)
        ...
        guard AXUIElementCopyAttributeValue(focused, kAXSelectedTextRangeAttribute as CFString, &rangeValue) == .success,
            ...
            throw ComputerUseError.message("[input.unsupported] The current selection cannot be read")
        }
        ...
        guard AXValueGetValue(rangeValue as! AXValue, .cfRange, &range),
            ...
            throw ComputerUseError.message("[input.unsupported] The current selection range is invalid")
        }
        guard try isSettableForSetValue(element: focused, attribute: kAXSelectedTextAttribute) else {
            throw ComputerUseError.message("[input.unsupported] This control does not support replacing the current selection")
        }
        let expected = (before as NSString).replacingCharacters(
        ...
        guard result == .success else {
            throw ComputerUseError.message("[input.write_failed] AXSelectedText returned \(result.rawValue); do not automatically replay")
        }
        return try verifiedInputResult(element: focused, expected: expected, query: query)
    }
    ...
    public func setValue(app query: String, elementIndex: String, value: String) throws -> ToolCallResult {
        let snapshot = try currentSnapshot(for: query)
        let element = try inputElement(snapshot: snapshot, index: elementIndex)
        _ = try inputText(element)
        ...
        guard result == .success else {
            throw ComputerUseError.message("[input.write_failed] AXValue returned \(result.rawValue); do not automatically replay")
        }
        return try verifiedInputResult(element: element, expected: value, query: query)
    }
    ...
    private func inputElement(snapshot: AppSnapshot, index: String) throws -> AXUIElement {
        ...
        try validateInputElement(element, pid: snapshot.app.pid)
        return element
    }

    private func validateInputElement(_ element: AXUIElement, pid: pid_t) throws {
        var actualPID: pid_t = 0
        guard AXUIElementGetPid(element, &actualPID) == .success, actualPID == pid else {
            throw ComputerUseError.message("[input.target_invalid] The target no longer belongs to the observed process (expected pid=\(pid), actual pid=\(actualPID))")
        }
    ...
    private func verifiedInputResult(element: AXUIElement, expected: String, query: String) throws -> ToolCallResult {
        guard try inputText(element) == expected else {
            throw ComputerUseError.message("[input.readback_mismatch] The value after writing differs from the requested text; do not automatically replay")
        }
        let snapshot = try refreshSnapshot(for: query)
        let result = snapshotResult(for: snapshot, style: .actionResult)
        return ToolCallResult(content: result.content + [.text("Input result: value_verified. Text readback matched; application-level effects are not verified.")])
    }
    ...
    private func performNonAXClickFallback(
        ...
        let eventPoint = inputEventPoint(fromScreenStatePoint: point)
        debugClickDecision("mouse_fallback pid=\(snapshot.app.pid) window=\(String(describing: snapshot.targetWindowID)) eventPoint=\(eventPoint) globalEnabled=\(globalPointerFallbacksEnabled(environment: ProcessInfo.processInfo.environment))")

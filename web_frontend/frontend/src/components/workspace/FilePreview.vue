...
const resolvedWorkspaceContext = computed<WorkspaceRequestContext>(() => {
  if (props.workspaceContext) return props.workspaceContext
  const payload = props.file.payload || {}
  return {
...
let rawFileObjectUrl = ''

async function loadRawPreview(): Promise<void> {
  releaseRawPreview()
  if (!props.file.path || !['image', 'pdf'].includes(previewKind.value)) return
  const scope = (props.file.scope || 'workdir') as WorkspaceScope
  const response = await workspaceApi.rawBlob(scope, props.file.path, resolvedWorkspaceContext.value)
  rawFileObjectUrl = URL.createObjectURL(response.blob)
  rawFileUrl.value = rawFileObjectUrl
}

function releaseRawPreview(): void {
  rawFileUrl.value = ''
...
async function handleDownload() {
  if (desktopFileActionsAvailable && props.file.path) {
    try {
      ...
      const destination = await saveWorkspaceFileAs(
        scope,
        props.file.path,
        resolvedWorkspaceContext.value,
      )
      if (destination) {
  ...
  if (props.file.path) {
    try {
      const scope = (props.file.scope || 'workdir') as WorkspaceScope
      const response = await workspaceApi.rawBlob(scope, props.file.path, resolvedWorkspaceContext.value)
      downloadBlob(response.blob, props.file.name)
    } catch (error) {
      message.error(error instanceof Error ? error.message : String(error))
    }
    return
  ...
  downloadBlob(blob, props.file.name)
}
...
async function handleReveal() {
  ...
  try {
    const scope = (props.file.scope || 'workdir') as WorkspaceScope
    await revealWorkspaceEntry(scope, props.file.path, resolvedWorkspaceContext.value)
  } catch (error) {
    message.error(error instanceof Error ? error.message : String(error))
  }
}
...
async function handleDelete() {
  if (!props.file.path) return
  const scope = (props.file.scope || 'workdir') as WorkspaceScope
  await workspaceApi.deleteFile(scope, props.file.path, resolvedWorkspaceContext.value)
...
async function addToReferences() {
  try {
    if (!props.file.path) return
    const scope = (props.file.scope || 'workdir') as WorkspaceScope
    const raw = await workspaceApi.rawBlob(scope, props.file.path, resolvedWorkspaceContext.value)
...
watch(() => [props.file.path, props.file.mimeType, props.file.kind], () => {
  void loadRawPreview().catch(error => message.error(error instanceof Error ? error.message : String(error)))
}, { immediate: true })

onBeforeUnmount(releaseRawPreview)

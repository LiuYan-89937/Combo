...
web_start_inference_ssh_tunnel() {
    ...
    if [[ "${AGENTFACTORY_INFERENCE_SSH_CHAT_LOCAL_PORT}" == "${AGENTFACTORY_INFERENCE_SSH_EMBEDDING_LOCAL_PORT}" \
        ...
        || "${AGENTFACTORY_INFERENCE_SSH_EMBEDDING_LOCAL_PORT}" == "${AGENTFACTORY_INFERENCE_SSH_TELEMETRY_LOCAL_PORT}" ]]; then
        web_fail "Chat, embedding, and telemetry SSH local ports must be different"
    fi

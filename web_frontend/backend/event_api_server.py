...
async def set_mcp_enabled(server_id: str, payload: dict[str, Any]):
    event = await _resource_command(
        "extensions_manage",
        {"action": "set_mcp_enabled", "server_id": server_id, "enabled": payload.get("enabled", True)},
        {"extension_config_updated"},
    )
    return {"event": event}
...
async def set_skill_enabled(skill_id: str, payload: dict[str, Any]):
    event = await _resource_command(
        "extensions_manage",
        {"action": "set_skill_enabled", "skill_id": skill_id, "enabled": payload.get("enabled", True)},
        {"extension_config_updated"},
    )
    return {"event": event}
...
async def scheduler_jobs():
    event = await _resource_command("scheduler_manage", {"action": "list"}, {"scheduler_jobs_listed"})
    return {"event": event}
...
async def create_scheduler_job(payload: dict[str, Any]):
    event = await _resource_command(
        "scheduler_manage",
        {"action": "create", "job": payload.get("job", payload)},
...
async def describe_scheduler_job(job_id: str):
    event = await _resource_command(
        "scheduler_manage",
        {"action": "describe", "job_id": job_id},
...
async def scheduler_runs(job_id: str | None = None, limit: int = Query(default=20, ge=1, le=200)):
    payload: dict[str, Any] = {"action": "runs", "limit": limit}
    if job_id:
...
async def run_scheduler_job_now(job_id: str):
    command = FactoryFrontendCommand(
        ...
        payload={"action": "run_now", "job_id": job_id},
    )
    await runtime_bridge.send_frontend_command(command)
    return {"accepted": True, "command": command.model_dump(mode="json")}

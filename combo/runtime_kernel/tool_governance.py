...
from combo.runtime_kernel.state import RuntimeState, ToolLoopGovernanceState, ToolLoopMetrics, ToolState
from combo.tooling.langgraph_node import tool_observation_message
from combo.tooling.spec import ToolLoopPolicyConfig
...
def _tool_policies(tools: Iterable[Any]) -> dict[str, ToolLoopPolicyConfig]:
    ...
    for tool in tools:
        ...
        combo = metadata.get("combo") if isinstance(metadata, dict) else None
        raw = combo.get("loop_policy") if isinstance(combo, dict) else None

...
class MainAgentCapabilityProfile:
    ...
    def __post_init__(self) -> None:
        ...
        normalized_mcp = tuple(dict.fromkeys(_required_text(value) for value in self.mcp_server_ids))
        object.__setattr__(self, "mcp_server_ids", normalized_mcp)
...
class MainAgentCapabilityProfileStore:
    ...
    def read(self) -> MainAgentCapabilityProfile:
        if not self._path.exists():
            return MainAgentCapabilityProfile(revision=1, capability_ids=(), mcp_server_ids=())
        ...
        mcp_server_ids = document.get("mcp_server_ids")
        if not isinstance(mcp_server_ids, list) or any(not isinstance(value, str) for value in mcp_server_ids):
            raise ValueError("main Agent capability profile mcp_server_ids must be an array of strings")

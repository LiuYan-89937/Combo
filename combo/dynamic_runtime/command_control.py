...
from combo.dynamic_runtime.dispatcher import CommandExecutionRegistry, CommandOutcome
from combo.dynamic_runtime.repositories import CommandInbox, MessageAlreadyBeingSteered
...
class CancelCommandRequestHandler:
    ...
    async def handle(
        ...
        if target.status == "queued":
            try:
                target = self._commands.cancel_queued_message(
                    command_id=target.command_id,
                    principal_id=envelope.principal_id,
                    session_id=envelope.session_id,
                )
            except MessageAlreadyBeingSteered:
                return CommandOutcome(status="rejected", rejection_code="message_already_being_steered")

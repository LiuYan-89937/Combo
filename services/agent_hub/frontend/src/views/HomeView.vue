...
const capabilities = computed(() => [
  { key: 'model', index: '01', icon: '/brand/combo/ui-icons/empty-model.png' },
  { key: 'skill', index: '02', icon: '/brand/combo/ui-icons/empty-skill.png' },
  { key: 'tool', index: '03', icon: '/brand/combo/ui-icons/empty-workspace.png' },
  { key: 'mcp', index: '04', icon: '/brand/combo/ui-icons/empty-mcp.png' },
  { key: 'scheduler', index: '05', icon: '/brand/combo/ui-icons/empty-scheduler.png' },
  { key: 'knowledge', index: '06', icon: '/brand/combo/ui-icons/empty-knowledge.png' },
])
...
.hero { position: relative; display: flex; min-height: max(720px, calc(100vh - var(--header-height))); align-items: center; overflow: hidden; padding: clamp(76px, 8vw, 116px) 0; }
.hero::before { position: absolute; inset: 0; background-image: radial-gradient(circle at 1px 1px, var(--border-strong) .8px, transparent 0); background-size: 40px 40px; mask-image: linear-gradient(105deg, black, transparent 68%); content: ''; opacity: .42; pointer-events: none; }
...
.hero__layout { position: relative; display: grid; grid-template-columns: minmax(0, .96fr) minmax(480px, 1.04fr); align-items: center; gap: clamp(52px, 7vw, 104px); }
...
.hero h1 { display: grid; justify-items: start; max-width: 760px; margin: 28px 0 28px; color: var(--text-strong); font-size: clamp(60px, 6.5vw, 96px); font-weight: 780; line-height: .94; letter-spacing: -.078em; }
.hero h1 > span:first-child { position: relative; z-index: 1; }
.hero-title__focus { position: relative; z-index: 0; display: inline-block; margin-top: .08em; padding: .09em .17em .13em .11em; border-radius: .14em; color: var(--on-primary); background: var(--primary); font-size: .82em; line-height: .9; letter-spacing: -.065em; transform: rotate(-.8deg); transform-origin: left center; }
.hero-title__focus::after { position: absolute; right: -.11em; bottom: .03em; width: .1em; height: .1em; border-radius: 50%; background: var(--primary); content: ''; }
...
.hero-scene { position: relative; min-height: 580px; }
.hero-scene__mascot { position: absolute; top: 49%; left: 50%; z-index: 1; display: grid; place-items: center; transform: translate(-50%, -50%); }
.hero-scene__mascot::before, .hero-scene__mascot::after { position: absolute; z-index: -1; bottom: 3%; height: 18px; border-radius: 50%; background: color-mix(in srgb, var(--text-strong) 18%, transparent); filter: blur(9px); content: ''; }
.hero-scene__mascot::before { left: 14%; width: 48%; }
.hero-scene__mascot::after { right: 7%; width: 23%; background: color-mix(in srgb, var(--text-strong) 15%, transparent); }
.hero-scene__bubble { position: absolute; top: 8%; left: 0; z-index: 2; display: flex; max-width: 350px; align-items: center; gap: 12px; padding: 15px 17px; border: 1px solid var(--border-strong); border-radius: 17px 17px 17px 4px; color: var(--text); background: var(--surface); box-shadow: var(--shadow-soft); font-size: 11px; line-height: 1.5; animation: float-soft 5.5s ease-in-out infinite; }
...
.section-head--split { display: grid; grid-template-columns: minmax(0, 1.15fr) minmax(300px, .55fr); align-items: end; gap: clamp(40px, 7vw, 104px); }
...
.capability-section { position: relative; border-block: 1px solid var(--border); background: var(--surface-subtle); }
.capability-composer { position: relative; display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; margin-top: 58px; }
.capability-card { position: relative; display: flex; min-height: 270px; flex-direction: column; padding: clamp(24px, 3vw, 34px); overflow: hidden; border: 1px solid var(--border-strong); border-radius: 24px; background: var(--surface); transition: transform 320ms var(--ease-out), box-shadow 320ms var(--ease-out); }
...
.capability-card img { width: 68px; height: 68px; object-fit: contain; filter: var(--brand-mark-filter); opacity: .88; }
.capability-card h3 { margin: auto 0 12px; color: var(--text-strong); font-size: clamp(26px, 3vw, 36px); letter-spacing: -.045em; }
.capability-card p { min-height: 3.44em; max-width: 430px; margin: 0; color: var(--text-secondary); font-size: 14px; line-height: 1.72; }

.control-section { color: var(--surface); background: var(--text-strong); }
...
.control-copy .eyebrow { color: color-mix(in srgb, var(--surface) 62%, transparent); }.control-copy h2 { margin: 13px 0 20px; color: var(--surface); font-size: clamp(44px, 5.4vw, 72px); line-height: 1; letter-spacing: -.06em; }.control-copy > p { margin: 0; color: color-mix(in srgb, var(--surface) 64%, transparent); font-size: 16px; line-height: 1.75; }
.control-points { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); border: 1px solid color-mix(in srgb, var(--surface) 20%, transparent); border-radius: 24px; background: color-mix(in srgb, var(--surface) 5%, transparent); }
.control-points article { display: grid; align-content: start; gap: 18px; min-height: 250px; padding: 26px 22px; }.control-points article + article { border-left: 1px solid color-mix(in srgb, var(--surface) 16%, transparent); }.control-points img { width: 42px; height: 42px; object-fit: contain; filter: invert(1); }.control-points article > div { display: grid; gap: 7px; margin-top: auto; }.control-points strong { color: var(--surface); font-size: 14px; }.control-points span { color: color-mix(in srgb, var(--surface) 62%, transparent); font-size: 11px; line-height: 1.65; }
:global([data-theme='dark']) .control-points img { filter: none; }

.download-section { padding: clamp(92px, 10vw, 148px) 0; }.download-shell { position: relative; display: grid; grid-template-columns: 210px minmax(280px, .75fr) minmax(420px, 1fr); align-items: center; gap: clamp(26px, 5vw, 70px); padding: clamp(40px, 5vw, 68px); border: 1px solid var(--border-strong); border-radius: 32px; background: var(--surface-subtle); }.download-mascot { position: relative; display: grid; place-items: center; }.download-mascot > * { grid-area: 1 / 1; }.download-mascot__halo { width: 180px; height: 180px; border: 1px solid var(--border); border-radius: 50%; box-shadow: 0 0 0 28px color-mix(in srgb, var(--text-strong) 2%, transparent); }.download-copy h2 { margin: 10px 0 15px; color: var(--text-strong); font-size: clamp(38px, 4.5vw, 62px); line-height: 1; letter-spacing: -.06em; }.download-copy p { margin: 0; color: var(--text-secondary); font-size: 13px; line-height: 1.7; }.download-count { display: block; margin-top: 16px; color: var(--text-muted); font: 9px/1 var(--font-mono); }.download-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }.download-grid article { position: relative; display: grid; gap: 18px; padding: 22px; border: 1px solid var(--border); border-radius: 20px; background: var(--surface); }.download-grid article.preferred { border-color: var(--text-strong); }.recommended { position: absolute; top: -10px; right: 15px; padding: 5px 8px; border-radius: 999px; color: var(--on-primary); background: var(--primary); font-size: 8px; }.download-platform { display: grid; gap: 2px; }.download-platform strong { color: var(--text-strong); font-size: 19px; }.download-platform span, .download-meta { color: var(--text-secondary); font-size: 10px; }.download-meta { display: flex; gap: 10px; font-family: var(--font-mono); }.download-steps { display: flex; grid-column: 2 / -1; justify-content: space-between; gap: 12px; color: var(--text-secondary); font-size: 9px; }.download-steps span { display: flex; align-items: center; gap: 7px; }.download-steps i { display: grid; width: 19px; height: 19px; place-items: center; border: 1px solid var(--border-strong); border-radius: 50%; color: var(--text-strong); font-style: normal; font-size: 8px; }.download-soon { grid-column: 3; margin: 0; color: var(--text-muted); font-size: 10px; }
...
@media (max-width: 1040px) {
  ...
  .capability-composer { grid-template-columns: repeat(2, minmax(0, 1fr)); }
...
@media (max-width: 720px) {
  ...
  .download-shell { grid-template-columns: 1fr; padding: 32px 20px; border-radius: 24px; text-align: center; }.download-mascot { display: none; }.download-grid { grid-template-columns: 1fr; text-align: left; }.download-steps { align-items: flex-start; flex-direction: column; width: fit-content; margin: 0 auto; text-align: left; }
}
